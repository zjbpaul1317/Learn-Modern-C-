...
public:
     ...
     * @param[in] time UTC时间
     * @param[in] thread_name线程名称
     */
    LogEvent(const std::string &logger_name, LogLevel::Level level,
             const char* file, int32_t line, int64_t elapse, uin32_t thread_id,
             unit64_t fiber_id, time_t time, const std:;string &thread_name);
     ...
     * @brief 获取协程id
     */
    unit64_t getFiberId() const { return m_fiberId; }
