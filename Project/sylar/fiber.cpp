...
namespace sylar
{
    ...
    /**
     * @brief 获取当前协程，同时充当初始化当前线程主协程的作用，此函数在使用协程之前需要调用
     */
     ...
     */
    Fiber::Fiber(std::function<void() cb, size_t stacksize, bool run_in_schduler)
        : m_id(s_fiber_id++)
        , m_cb(cb)
    ...
    /**
     * @brief 线程的主协程析构时需要特殊处理，因为主协程没有分配栈和cb
     */
    Fiber::~Fiber()
    {
        SYLAR_LOG_DEBUG(g_logger) << "Fiber::~Fiber() id = " <<m_id;
    ...
    /**
     * @brief 为了简化状态管理，强制只有TERM状态的协程才可以重置，但其实刚创建好但没执行过的协程也应该允许重置的
     */
